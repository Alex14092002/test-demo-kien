...
(async ()=>{
  ...
  const res = await fetch(`https://data-kieh-default-rtdb.firebaseio.com/combo3.json`)
  const data = await res.json()
  console.log(data);
  if(data){
    Object.entries(data).map(([key , value])=>{
      ...
      <div class="img-product">
         ...
         <div class="img-hover mini-product">
            ...
            <button class="btn-view-more btn-mini btn-xem-nhanh-cart" onclick="showPopup('combo3' ,'${encodeURIComponent(key)}')"  >XEM NHANH</button>
         </div>
      </div>
